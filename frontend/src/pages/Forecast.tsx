...
export default function Forecast() {
  ...
  const handleExportCSV = () => {
    if (!data) return;
    const { dates, mean, lower, upper } = data.forecast;
    let csvContent = "data:text/csv;charset=utf-8,Date,Forecast,Lower_80,Upper_80\n";
    
    for (let i = 0; i < dates.length; i++) {
      const row = [dates[i], mean[i], lower[i], upper[i]].join(",");
      csvContent += row + "\n";
    }
    ...
    link.setAttribute("download", `Laplace_${data.model}_Forecast.csv`);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-2">Forecast & Export</h1>
