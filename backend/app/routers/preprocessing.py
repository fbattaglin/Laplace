import os
import pandas as pd
import logging
from fastapi import APIRouter, HTTPException
from app.models.schemas import CleanRequest, AnomalyRequest
from app.services.cleaning import run_cleaning_pipeline
from app.services.anomalies import run_anomaly_detection

logger = logging.getLogger("laplace.routers.preprocessing")
router = APIRouter(prefix="/api")

DATA_DIR = "data"
UPLOAD_DIR = os.path.join(DATA_DIR, "uploads")

@router.post("/clean")
def clean_data(req: CleanRequest):
    if req.dataset_type == "reference":
        file_path = os.path.join(DATA_DIR, f"{req.dataset_name}.csv")
    else:
        file_path = os.path.join(UPLOAD_DIR, req.dataset_name)
        
    if not os.path.exists(file_path):
        logger.error(f"Cleaning target dataset not found: {req.dataset_name}")
        raise HTTPException(status_code=404, detail="Dataset not found")
        
    try:
        if file_path.endswith('.csv'):
            df = pd.read_csv(file_path)
        else:
            df = pd.read_excel(file_path)
            
        logger.info(f"Running cleaning pipeline for: {req.dataset_name}")
        from app.main import sanitize_float_values
        return sanitize_float_values(run_cleaning_pipeline(df, req.date_col, req.target_col, req.config))
    except Exception as e:
        logger.error(f"Cleaning execution failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/anomalies/detect")
def detect_anomalies(req: AnomalyRequest):
    if req.dataset_type == "reference":
        file_path = os.path.join(DATA_DIR, f"{req.dataset_name}.csv")
    else:
        file_path = os.path.join(UPLOAD_DIR, req.dataset_name)
        
    if not os.path.exists(file_path):
        logger.error(f"Anomaly target dataset not found: {req.dataset_name}")
        raise HTTPException(status_code=404, detail="Dataset not found")
        
    try:
        if file_path.endswith('.csv'):
            df = pd.read_csv(file_path)
        else:
            df = pd.read_excel(file_path)
            
        logger.info(f"Detecting anomalies for: {req.dataset_name} using method: {req.method}")
        from app.main import sanitize_float_values
        return sanitize_float_values(run_anomaly_detection(df, req.date_col, req.target_col, req.method, req.threshold))
    except Exception as e:
        logger.error(f"Anomaly detection failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
